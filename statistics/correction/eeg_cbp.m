function [cbp_h, cr, perm] = eeg_cbp(alldata, spatialind, m, sides, ...
    feature_alpha, cluster_alpha, whichstat, method, iteration)
% t-test cluster based correction
%
% [cbp_h, cr, perm] = eeg_cbp(alldata, spatialind, m, sides, ...
%     feature_alpha, cluster_alpha, whichstat, method, iteration)
% alldata: subj*feature1(*feature2); e.g. subj*tp or subj*tp*freq
% spatialind: spatial electrode index
...
fn = @(h) label_clusters(h,spatialind(:,2));
[cr.clustersize, cr.clusterstat, cr.labels] = cluster_stats(cr.stat, cr.h, cr.cluster_calc_stat_method,fn);
