function [cfg] = ft_databrowser_hyhver(cfg, data)
...
cfg.trialboundary       = ft_getopt(cfg, 'trialboundary', 'on'); %$$ Modified by Yiheng Hu (2023.9.1)
...
if ~isempty(event) && isstruct(event)
  ...
  eventcolors = colorcheck('rbgmcyk', numel(eventtypes)); %$$ Modified by Yiheng Hu (2023.9.1)
...
trials_info = opt.trlorg(opt.trlorg(:,2)<=endsample,:); %$$ Updated by Yiheng Hu (2024.5.20)
...
%$$ Modified by Yiheng Hu (2023.9.1)
% plot trial boundary
if istrue(cfg.trialboundary) && istrue(cfg.continuous)
    % plot a line or a box for each of the events
    for i = 1:size(trials_info,1)
        ft_plot_line((trials_info(i,2))/opt.fsample*ones(1,2), [-1 1], 'tag', 'event', 'hpos', opt.hpos, 'vpos', opt.vpos, ...
            'width', opt.width, 'height', opt.height, 'hlim', opt.hlim, 'vlim', [-1 1], 'color', [0 0 0]);
